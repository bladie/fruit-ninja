...
con = sql.connect(host="localhost", user="root", password="",
                      database="python")  # Put in your own password and create a database of the same name too.
cur = con.cursor()
import sys
import pygame
import random
...
def menu():
    ...
    while True:
        ...
        font5 = pygame.font.SysFont("Elephant", 25)
        text5 = font5.render('Enter name', True, black)
        b5 = pygame.Rect(210, 325, 170, 60)
        text5Rect = text5.get_rect()
        text5Rect.center = (295, 345)
        pygame.draw.rect(home_page, (255, 222, 173), b5)

        home_page.blit(text1,text1Rect)
        home_page.blit(text2,text2Rect)
        home_page.blit(text3,text3Rect)
        home_page.blit(text4,text4Rect)
        home_page.blit(text5,text5Rect)
...
def game2():
    ...
    while running:
        ...
        for hit in hits:  # new mobs generated when bullet hits them
            ...
            all_sprites.add(m)
            mobs.add(m)
            up='''update score_card
            set score=score+1
            '''
            cur.execute(up)
            con.commit()
            print(input_box1, score)
...
def Name():
    ...
    X = 580
    Y = 640
    Names = pg.display.set_mode((X, Y)) # name input variable
    pg.display.set_caption("FRUIT  WARRIOR")
    image = pg.image.load("wood.png")
    pg.init()
    screen = pg.display.set_mode((580, 640))
    COLOR_INACTIVE = pg.Color('lightskyblue3')
    COLOR_ACTIVE = pg.Color('dodgerblue2')
    FONT = pg.font.Font(None, 48)
    Names.fill(white)
    Names.blit(image, (0, 0))
    score_data = '''insert into SCORE_CARD values
                           ('{}',{})'''.format(input_box1, score)
    cur.execute(score_data)
    con.commit()
    class InputBox:
    ...
    def main():
        clock = pg.time.Clock()
        input_box1 = InputBox(140, 210, 140, 56) #size of input box and positioning
